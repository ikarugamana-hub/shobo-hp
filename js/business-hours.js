// 営業日判定の共通ロジック（index/kansen/story/sp の4ファイルで共有）
// 2031年以降の祝日を追加する場合は、この配列を更新してください
// ※春分の日・秋分の日は国立天文台が前年2月の官報で正式確定します。2029・2030年分は現時点の推定値です（確定後に要確認）
var HOLIDAYS = ['2026-01-01','2026-01-12','2026-02-11','2026-02-23','2026-03-20','2026-04-29','2026-05-03','2026-05-04','2026-05-05','2026-05-06','2026-07-20','2026-08-11','2026-09-21','2026-09-22','2026-09-23','2026-10-12','2026-11-03','2026-11-23','2027-01-01','2027-01-11','2027-02-11','2027-02-23','2027-03-21','2027-03-22','2027-04-29','2027-05-03','2027-05-04','2027-05-05','2027-07-19','2027-08-11','2027-09-20','2027-09-23','2027-10-11','2027-11-03','2027-11-23','2028-01-01','2028-01-10','2028-02-11','2028-02-23','2028-03-20','2028-04-29','2028-05-03','2028-05-04','2028-05-05','2028-07-17','2028-08-11','2028-09-18','2028-09-22','2028-10-09','2028-11-03','2028-11-23','2029-01-01','2029-01-08','2029-02-11','2029-02-12','2029-02-23','2029-03-20','2029-04-29','2029-04-30','2029-05-03','2029-05-04','2029-05-05','2029-07-16','2029-08-11','2029-09-17','2029-09-23','2029-09-24','2029-10-08','2029-11-03','2029-11-23','2030-01-01','2030-01-14','2030-02-11','2030-02-23','2030-03-20','2030-04-29','2030-05-03','2030-05-04','2030-05-05','2030-05-06','2030-07-15','2030-08-11','2030-08-12','2030-09-16','2030-09-23','2030-10-14','2030-11-03','2030-11-04','2030-11-23'];
// 祝日データが網羅している最終年。この年を過ぎたら祝日判定ができないため、
// 「受付中」と誤表示しないよう holidayDataCoversToday() で判定してUIを中立表示に切り替える。
// HOLIDAYS を延長したら、この値も必ず更新してください。
var HOLIDAY_DATA_UNTIL = 2030;

function tokyoNow(){
  return new Date(new Date().toLocaleString('en-US', {timeZone: 'Asia/Tokyo'}));
}

// 祝日データが今日をカバーしているか。false の場合、isOpen() の結果は
// 祝日を考慮できていないため、リアルタイムの受付状況として表示してはいけない。
function holidayDataCoversToday(){
  return tokyoNow().getFullYear() <= HOLIDAY_DATA_UNTIL;
}

function isOpen(){
  var d = tokyoNow();
  var day = d.getDay();
  if (day === 0 || day === 6) return false;
  var mm = ('0' + (d.getMonth() + 1)).slice(-2);
  var dd = ('0' + d.getDate()).slice(-2);
  var ymd = d.getFullYear() + '-' + mm + '-' + dd;
  if (HOLIDAYS.indexOf(ymd) !== -1) return false;
  var md = mm + '-' + dd;
  if (md >= '08-13' && md <= '08-15') return false;   // お盆
  if (md >= '12-29' || md <= '01-03') return false;   // 年末年始
  var h = d.getHours();
  return h >= 10 && h < 17;
}
